...
- Pagination follow-ups (`next-search-page` and friends) need a session to hang state on, which over HTTP means the `mcp-session-id` header. Clients running `--stateless-http`, or speaking a protocol version that dropped sessions, get a fresh state slot per request and have to re-run the original search instead. Over stdio the whole process is one client, so follow-ups always work.
