...
- Pagination follow-ups (`next-search-page` and friends) need a transport session to hang state on: the `mcp-session-id` header on streamable HTTP, or the `session_id` the SSE endpoint issues. Two cases have no such session — servers started with `--stateless-http`, where the transport passes the id through without checking it, and clients speaking a protocol version that dropped sessions. The server will not partition state on an id it cannot trust, so those callers get a fresh slot per request and have to re-run the original search instead. Over stdio the whole process is one client, so follow-ups always work.
